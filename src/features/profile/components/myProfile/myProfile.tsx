...
const getValidation = (data: UpdateProfileInterface) => ({
  ...
  postalCode: isMissing(data?.postalCode) ? required("Le code postale") : data?.postalCode.length < 5 ? notValid("Le code postale") : null,
    country: isMissing(data?.country) ? required("Le pays") : null,
  email: isMissing(data?.email)
...
const MyProfile = () => {
  ...
  const [updateValidation, setUpdateValidation] = useState(
    getValidation(connectedUser as UpdateProfileInterface)
  );
  const [value, setValue] = useState(connectedUser?.country);
  ...
  return (
    <div>
      <div className="profileHeaderContainer">
        ...
        {!isUpdate && (
          <span className="update" onClick={() => prepareUpdateForm(true)}>
            Modifier
          </span>
        )}
      </div>
        ...
        {isUpdate && (
          <div className={"updateButtons"}>
            <button
              disabled={
                !(
                  Object.values(updateValidation).filter((item) => !!item)
                    .length === 0
                )
              }
            ...
            <button
              onClick={() => prepareUpdateForm(false)}
              className={"btn success cancel"}
            >
              Annuler
            </button>
          </div>
...
export const AvatarWithName: React.FC<{ withMargin?: boolean }> = ({
  withMargin,
}) => {
  const connectedUser = useAppSelector(selectConnectedUser);
  return (
    <div className={`avatarWithName ${withMargin ? "withMargin" : ""}`}>
      ...
      <div className="desc">
        <span className="name">{`${capitalizeFirstLetter(
          connectedUser?.firstName as string
        )} ${capitalizeFirstLetter(connectedUser?.lastName as string)}`}</span>
