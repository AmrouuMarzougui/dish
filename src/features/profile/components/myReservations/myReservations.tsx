...
import { deleteOrder } from "../../../../app/store/storeModules/cart/cartService";
...
const MyReservations = () => {
  ...
  const getMargin = () => {
    ...
    if (tabSelected === "2")
      return { marginLeft: deviceWidth > 768 ? "150px" : "50%" };
  };
  ...
  return (
    <div>
      ...
      <div className="cardsContainer">
        {(tabSelected === "1" ? orders.newOrders : orders?.oldOrders).map(
          (item: any, index: number) => (
            <div key={index} className="cardContainer">
              ...
              <div
                className={
                  tabSelected === "2"
                    ? "detailCardCont detailCardContdisabled"
                    : "detailCardCont"
                }
              >
                           ...
                           deleteOrder({ id: item?._id })
                             .then((result) => {
                              getMyReservationsList().then((res: any) => {
                               ...
                               swal("Réservation supprimé avec succées", {
                                 icon: "success",
                               });
                             })
                             .catch((err) =>
                               swal("Réservation nos supprimé!!", {
                                 icon: "danger",
                               })
                             );
                         }
                ...
                <div>
                  <div className="inlineDetails">
                    ...
                    <span>
                      {moment(item?.orderedForDate).format(
      ...
      {(tabSelected === "1" ? orders.newOrders : orders?.oldOrders).length ===
        0 && (
        <EmptyMessage
          config={{
