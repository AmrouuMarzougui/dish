...
const ChangePassword = () => {
  ...
  const submitForm = (e: any) => {
    e.preventDefault();
    verifyPassword({ password: form.current.password })
      .then((res: any) => {
  ...
  const STEPS: Array<Array<JSX.Element>> = [
    [
      ...
      <div style={{ margin: " 0 0 20px 0" }} className={"profileInputs"}>
        <AvatarWithName />
      </div>,
      <InputField
        key={1}
    ...
    [
      ...
      >
        Vous allez modifier le mot de passe de <b>{connectedUser?.email}</b>
      </span>,
      <InputField
        key={2}
        config={{
