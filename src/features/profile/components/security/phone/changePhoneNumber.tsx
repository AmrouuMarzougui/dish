...
const ChangePhoneNumber = () => {
  ...
  const STEPS: Array<Array<JSX.Element>> = [
    [
      ...
      //   <div>
      //     <label>Saisissez le code que nous avons envoyé au numéro de téléphone se terminant par {connectedUser?.telephone?.toString()?.slice(-2) as string}</label>
      ...
      <InputField
        ...
        config={{
          ...
          label:
            "Saisissez le code que nous avons envoyé au numéro de téléphone se terminant par ",
          placeholder: "SMS_VERIFICATION.FOUR_DIGITS_CODE",
          type: "number",
          onInit: () =>
            confirmPhoneNumber({
              phone: connectedUser?.telephone.toString() as string,
            }).then((res: any) => (confirmPhone.current.old = res.data)),
          onChange: (value: string) => {
            setValidButton(
              value.length > 0 &&
                value.toString() === confirmPhone.current.old.toString()
            );
          },
          rules: {
            minLength: 4,
    ...
    [
      ...
      <PhoneNumberInput
        ...
        config={{
          label:
            "Nous vous enverrons un code de validation unique par SMS pour confirmer votre numéro",
          ...
          onChange: (value: { number: number; country: any }) => {
            if (form.current) {
              ...
              setValidButton(
                !isNaN(value.number) && value.number.toString().length > 6
              );
            }
          },
        }}
    ...
    [
      ...
      <InputField
        ...
        config={{
          ...
          onInit: () =>
            confirmPhoneNumber({
              phone: `${selectedCountry?.dialCode}${form.current?.telephone}`,
            }).then((res: any) => (confirmPhone.current.new = res.data)),
          onChange: (value: string) => {
            setValidButton(
              value.length > 0 &&
                value.toString() === confirmPhone.current.new.toString()
            );
          },
        }}
      />,
  ...
  return (
    <div>
      ...
      <button
        ...
        className={`btn ${validButton ? "success" : "error"}`}
        style={{ width: "100%" }}
        onClick={toNextStep}
      >
        {t(buttonText)}
      ...
      <span
        ...
        className={"labelCancel clickable"}
      >
        Annuler
      </span>
    </div>
...
export const Hint: React.FC<{
  config: { text: string; type: "info" | "danger" };
}> = ({ config }) => {
  return (
