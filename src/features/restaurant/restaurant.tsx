...
import GoogleMapApiMultipe from "./components/restaurantMap/MapRestaurant";
import Map from "./components/restaurantMap/MapRestaurant";
import GoogleMapApi from "./components/restaurantMap/MapRestaurant";
...
const Restaurant = () => {
  ...
  const favoriteCondition = () =>
    connectedUser?.favoriteRestaurants?.find(
      (item: any) =>
        item?._id ===
        (selectedRestaurant?._id || selectedRestaurant?.restaurantId)
    );
  useEffect(() => {
  ...
  const setFav = () => {
    setFavoriteRestaurant({
      id: selectedRestaurant?._id || selectedRestaurant?.restaurantId,
  ...
  useEffect(() => {
    ...
    getRestaurantById({ id: window.location.pathname.slice(12) })
      .then((res: any) => {
        console.log(res);
  ...
  const getMargin = () => {
    if (tabSelected === "DISHES")
      return { marginLeft: deviceWidth > 919 ? "150px" : "25%" };
    if (tabSelected === "DESSERTS")
      return { marginLeft: deviceWidth > 919 ? "300px" : "50%" };
    if (tabSelected === "DRINKS")
      return { marginLeft: deviceWidth > 919 ? "450px" : "75%" };
  };

  useEffect(() => {
    isMounted.current
      ? document
          .getElementById(tabSelected)
    ...
    if (!orderDetails || orderDetails === "null") {
      ...
      dispatch(
        pushToToastsArray({
          ...
          message:
            orderDetails?.restaurant._id === selectedRestaurant?._id
              ? "Une réservation existe déjà"
              : "Vous avez une autre commande en cours",
          type: "info",
        })
      );
    }
  ...
  useEffect(() => {
    getSection({ id: window.location.pathname.slice(12) })
      .then((res: any) => {
        console.log(res);
        setSelectedSectionList(res?.data);
      })
      .catch((err) => {
        console.log("err", err);
      });
  ...
  return selectedRestaurant ? (
    <div style={{ position: "relative" }}>
      ...
      <div
        className="headerRestaurant"
        style={{
          backgroundImage: `url(${selectedRestaurant?.imageUrl})`,
      ...
      >
          ...
          <div className="details">
            <span className="rateComments">
              ...
              <span
                ...
                onClick={() =>
                  navigate(`/restaurantRates/${selectedRestaurant?._id}`)
                }
              >
            ...
            <span>
              {selectedRestaurant?.specialty?.map(
        ...
        {deviceWidth > 626 && (
          <div className="mapContainer">
            <GoogleMapApi
              el={selectedRestaurant && selectedRestaurant}
              title={selectedRestaurant?.name}
              info={selectedRestaurant?.description}
              ratingLength={selectedRestaurant?.rating.length}
              id={selectedRestaurant?._id}
              lat={selectedRestaurant.location?.coordinates[1]}
              lng={selectedRestaurant.location?.coordinates[0]}
              defaultZoom={20}
              localisation={selectedRestaurant?.address}
              phone={selectedRestaurant?.phoneNumber}
              image={selectedRestaurant?.imageUrl}
              avergePrice={selectedRestaurant?.avgPrice[0]}
              globalRating={selectedRestaurant?.globalRating}
              speciality={selectedRestaurant?.specialty?.map(
                (item: any, index: number) =>
                  `${index !== 0 ? " • " : ""}${item}`
              )}
          ...
          // <RestaurantMap item={selectedRestaurant} id={"restaurantPosition"} />
          // <GoogleMapApiMultipe />
          //   <GoogleMapApiMultipe
          //   googleMapURL="https://maps.googleapis.com/maps/api/js?key=AIzaSyCIiaTseEf-xGTLJyBrl1r93wBSdCUzG80&v=3.exp&libraries=geometry,drawing,places"
          //   loadingElement={<div style={{ height: `100%` }} />}
          //   containerElement={<div style={{ height: `100%` }} />}
          //   mapElement={<div style={{ height: `100%` }} />}
          // />
      ...
      {deviceWidth < 920 && selectedRestaurant?.type === "RESERVATION" && (
        <div className="restaurantTabsContainer">
          <div
            style={{ padding: "0 5vw", display: "flex", justifyContent: "end" }}
            className="tabs"
          >
            <button
      ...
      <div style={{ padding: "18px 5vw" }}>
        <div className="restaurantTabsContainer">
          <div className="tabs">
            <div className="cont">
              <span
                onClick={() => setTabSelected("ENTRIES")}
                className={tabSelected === "ENTRIES" ? "active tab" : "tab"}
              >
                {t("RESTAURANT.ENTRIES")}
              </span>
              <span
                onClick={() => setTabSelected("DISHES")}
                className={tabSelected === "DISHES" ? "active tab" : "tab"}
              >
                {t("RESTAURANT.DISHES")}
              </span>
              <span
                onClick={() => setTabSelected("DESSERTS")}
                className={tabSelected === "DESSERTS" ? "active tab" : "tab"}
              >
                {t("RESTAURANT.DESSERTS")}
              </span>
              <span
                onClick={() => setTabSelected("DRINKS")}
                className={tabSelected === "DRINKS" ? "active tab" : "tab"}
              >
                {t("RESTAURANT.DRINKS")}
              </span>
            </div>
            {deviceWidth > 919 && selectedRestaurant?.type === "RESERVATION" && (
              <button className="btn success" onClick={() => reserveTable()}>
                Réservez une table
              </button>
            )}
          </div>
      ...
      <BottomSheet
        ...
        open={
          (moreDetail.isOpen && moreDetail.mobile) ||
          (shoppingModal.isOpen && shoppingModal.mobile)
        }
      >
        {moreDetail.isOpen && (
          <MoreDetailModal
