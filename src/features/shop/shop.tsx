...
const Shop = () => {
  ...
  const decIncProduct = (data: any) => {
    setLocalLoader(true);
    decrementIncrementProductInCart(data)
      .then(() => {
        getCart({ isSilent: true })
          .then((res: any) => {
            dispatch(setCart(res.data));
  ...
  const cartHasProducts = () =>
    !!cart &&
    (cart?.articles?.filter((item: any) => !!item?.article)?.length > 0 ||
      cart?.boisson?.filter((item: any) => !!item?.article)?.length > 0);

  const shoppingCart = (
    <div className="shoppingCart" id="justAnId">
      <button
        disabled={!cartHasProducts()}
        className={`btn ${cartHasProducts() ? "success" : ""}`}
                ...
                <div className="cont">
                  <span className="title">
                    <div className={`incrementDecrement`}>
                      <img
                        onClick={() =>
                          item?.numbers === 1 || localLoader
                            ? null
                            : decIncProduct({
                                platId: item?.article?._id,
                                increment: false,
                              })
                        }
                        className={
                          item?.numbers === 1 ? "unClickable" : "clickable"
                        }
                        draggable={false}
                        src={item?.numbers === 1 ? moinInActive : moinActive}
                        alt=""
                      />
                      {item?.numbers}
                      <img
                        onClick={() =>
                          item?.numbers === 10 || localLoader
                            ? null
                            : decIncProduct({
                                platId: item?.article?._id,
                                increment: true,
                        ...
                        className={
                          item?.numbers === 100 ? "unClickable" : "clickable"
                        }
                        draggable={false}
                        src={item?.numbers === 100 ? plusInActive : plusActive}
                        alt=""
                      />
                    </div>
                    {item?.article?.name}
                  ...
                  <span
                    className="delete"
                    onClick={() => deleteItem({ platId: item?.article?._id })}
                  >
                    Supprimer
                  </span>
                </div>
                {/* <span style={{ whiteSpace: 'nowrap' }}>{item?.article?.price * item?.numbers.toFixed(2)}€€€ €</span> */}
                  ...
                  <span className="title">
                    <div className={`incrementDecrement`}>
                      ...
                      <img
                        ...
                        className={
                          item?.numbers === 10 ? "unClickable" : "clickable"
                        }
                        draggable={false}
                        src={item?.numbers === 10 ? plusInActive : plusActive}
                        alt=""
                      />
                    </div>
                    {item?.article?.name}
