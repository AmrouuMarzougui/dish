...
const Login = () => {
  ...
  const toNextStep = () => {
    if (stepIndex < loginSteps.length - 1) {
      verifyEmail({ email: loginForm.current.email })
        .then((res: any) => {
        ...
        .catch((err) => {
          if (err.code === 404) {
            ...
            dispatch(
              pushToToastsArray({
                uniqueId: generateUniqueId(),
                message: "Votre identifiantest incorrecte",
                type: "danger",
              })
            );
          }
  ...
  return (
    <div>
      ...
      <button
        ...
        className={`btn ${validButton && "success"}`}
        style={{ width: "100%" }}
        onClick={toNextStep}
      >
        {t("NEXT")}
      ...
      {stepIndex === 0 ? (
        ...
        <span className="getConnected">
          <span
            className="link"
            onClick={() => navigate(Paths.auth.passwordForgotten)}
          >
            {t("PASSWORD.HINT")}
          </span>
        </span>
...
const ConnectWithGoogleOrFacebook: React.FC<{ loginReq: Function }> = ({
  loginReq,
}) => {
  const { t } = useTranslation();
    ...
    <div>
      ...
      <div className="socialMediaContained">
        ...
        <div ref={facebookRef} style={{ display: "none" }}>
          <FacebookLogin appId="1093217634617503" callback={responseFacebook} />
        </div>
      </div>
    </div>
  );
