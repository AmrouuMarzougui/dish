...
const Register: React.FC<{}> = () => {
  ...
  const registerSteps: Array<Array<JSX.Element>> = [
    ...
    [
      <InputField
        ...
        config={{
          ...
          onInit: () => {
            setValidButton(
              registerForm.current.lastName.length > 0 &&
                registerForm.current.firstName.length > 0
            );
          },
          onChange: (value: string) => {
            registerForm.current.lastName = value;
            setValidButton(
              value.length > 0 && registerForm.current.firstName.length > 0
            );
          },
        }}
      />,
      <InputField
        ...
        config={{
          ...
          onChange: (value: string) => {
            ...
            setValidButton(
              value.length > 0 && registerForm.current.lastName.length > 0
            );
          },
        }}
      />,
    ...
    [<Congrats key={7} name={registerForm.current.firstName} />],
  ];
  const toNextStep = () => {
    if (stepIndex < registerSteps.length - 1) {
      console.log(stepIndex);
      if (stepIndex === registerSteps.length - 2) {
        registerRequest(registerForm.current)
          .then((res) => {
      ...
      loginRequest(
        {
          email: registerForm.current.email,
          password: registerForm.current.password,
        },
        authEndpoints.login
      ).then((res) => {
        navigate(Paths.home);
  ...
  return (
    <div className="registerFirstStepContainer">
      {stepIndex !== registerSteps.length - 1 && (
        <label className="boldLabelTitle">{t("REGISTER.HINT")}</label>
      )}
      <div>
...
const Congrats: React.FC<{ name: string }> = ({ name }) => {
  ...
  return (
    <div className="congratsContainer">
      ...
      <span>{t("REGISTER.ACCOUNT_CREATED_SUCCESSFULLY")}</span>
    </div>
  );
};
