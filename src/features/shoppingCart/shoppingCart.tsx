...
const ShoppingCart = () => {
  ...
  useEffect(() => {
    getPaymentMethodById({ id: orderDetails.restaurant?._id })
      .then((res: any) => {
        console.log(res);
  ...
  const payment = () => {
    if (cart !== null || cart?.subTotalPrice.toFixed(2) !== 0) {
      if (!orderDetails || orderDetails === "null") {
        ...
        if (!query.get("redirect_status") && !query.get("payment_intent")) {
          passOrder({
            ...
            isCancelled: false,
            isPaid: false,
            orderedForDate: `${moment(orderDetails.date).format(
              "YYYY-MM-DD"
            )}T${orderDetails.time || "00:00"}:00`,
            panierId: cart?._id,
  ...
  const setDeliveryDetails = () => {
    ...
    if (deliveryForm.paymentMethod === "hand") {
      passOrder({
        ...
        isCancelled: false,
        isPaid: false,
        orderedForDate: `${moment(orderDetails.date).format("YYYY-MM-DD")}T${
          orderDetails.time || "00:00"
        }:00`,
        panierId: cart?._id,
    ...
    <div style={{ position: "relative" }}>
      <NavBar
        config={{
          ...
          rightComponent:
            deviceWidth > 768 ? (
              <NavBarRightComp disableRestaurantBtn={true} />
            ) : undefined,
      ...
      <div className="shoppingCartContainer">
        ...
        orderDetails?.restaurant?.type === "RESERVATION" ? (
          <span className="title">
            Réservation • {orderDetails?.restaurant?.name} •{" "}
            {orderDetails?.peopleNumber} Pers.{" "}
            {moment(orderDetails?.date).format("MMM Do YYYY")} •{" "}
            {orderDetails?.time}
          </span>
        ) : (
          orderDetails?.restaurant && (
            <span className="title">
              Livraison • {orderDetails?.restaurant?.name}{" "}
            </span>
          )
        )}
        ...
        <div className="mainCont">
          {isDelivery === 1 && (
            <div className={"paymentContainer"}>
              ...
              <div
                className="radioEl"
                onClick={() => changedDeliveryForm("hand", "paymentMethod")}
              >
                <div className="flexRadio">
                  <span
                    className={`radio ${
                      deliveryForm.paymentMethod === "hand" && "isActive"
                    }`}
                  >
                    <span
              ...
              <div
                className="radioEl"
                onClick={() => changedDeliveryForm("CB", "paymentMethod")}
              >
                <div className="flexRadio">
                  <span
                    className={`radio ${
                      deliveryForm.paymentMethod === "CB" && "isActive"
                    }`}
                  >
                    <span
                      className={`innerRadio ${
              ...
              <div className="input">
                ...
                <input
                  ...
                  onChange={(e) =>
                    changedDeliveryForm(e.target.value, "deliveryAddress")
                  }
              ...
              <div className="input">
                ...
                <input
                  ...
                  style={{ marginTop: "10px" }}
                  onChange={(e) =>
                    changedDeliveryForm(e.target.value, "instruction")
                  }
                  className="address whiteBack"
                  type="text"
                />
              ...
              <button
                ...
                disabled={!getDeliveryFormDisabledBtn()}
                className={`btn ${
                  getDeliveryFormDisabledBtn() ? "success" : "simple"
                }`}
              >
          ...
          <div className="shoppingContainer">
            <span
              className="addArticles clickable"
              onClick={() =>
                navigate(orderDetails?.restaurant ? Paths.shop : Paths.home)
              }
            >
              + Ajouter des articles
            </span>
            <div className="horizontalSeparator" />
            {cart && cart?.subTotalPrice.toFixed(2) > 0 ? (
              <div>
                    ...
                    cart?.articles?.map((item: any) =>
                      item?.article?.name ? (
                        <div className="shoppingItem" key={item?.article?._id}>
                          <div className="cont">
                            <span className="title">
                              {item?.numbers} {item?.article?.name}
                            </span>
                            {item?.options &&
                              item?.options?.length > 0 &&
                              item.options.map((t: any, index: number) => (
                            ...
                            {/* {
                                  item?.options && item?.options?.length > 0 && (
                                    <div className={'desc'}>
                                      ...
                                      &nbsp;
                                      &nbsp;
                                      <span>{item?.article?.options[0].price.toFixed(2)} €</span>
                                    </div>
                                  )
                                } */}
                            ...
                            <span
                              className="delete"
                              onClick={() =>
                                deleteItem({ platId: item?.article?._id })
                              }
                            >
                              Supprimer
                            </span>
                          ...
                          <span style={{ whiteSpace: "nowrap" }}>
                            {(item?.article?.price * item?.numbers).toFixed(2)}{" "}
                            €
                          </span>
                        </div>
                      ) : (
                        ...
                        <div className="shoppingItem" key={item?.article?._id}>
                          <div className="cont">
                            ...
                            <span
                              className="delete"
                              onClick={() =>
                                deleteItem({ boissonId: item?.article?._id })
                              }
                            >
                              Supprimer
                            </span>
                ...
                <div className="shoppingResult">
                  ...
                  {/* <span>{cart?.subTotalPrice : "0.00"} €</span> */}
                  <span>{cart ? cart.subTotalPrice.toFixed(2) : "0.00"} €</span>
                </div>
              </div>
            ) : (
              <EmptyMessage
                config={{ title: "Aucun article est séléctionner", text: "" }}
              />
            )}
          </div>
          {(isDelivery === 0 || isDelivery === 2) && orderDetails?.restaurant && (
            <div className="paymentContainer">
              ...
              <div
                className="horizontalSeparator"
                style={{ margin: "35px 0 20px 0" }}
              />

              {paymentMethodList &&
                paymentMethodList.map((paymentMethod: any, index: any) =>
                  paymentMethod?.moreInfos.paymentMethods.map((k: any) => (
                    <div key={index}>
                      <div className="radioEl">
                        <div className="flexRadio">
                          ...
                          <span className="radioLabel">{k}</span>
                        </div>
                      </div>
                      <div
                        className="horizontalSeparator"
              ...
              <button
                disabled={!cart || cart.subTotalPrice.toFixed(2) === 0}
                className={`btn ${
                  !!cart && cart.subTotalPrice.toFixed(2) > 0 ? "success" : ""
                }`}
                onClick={payment}
              >
                Passer au paiement
              ...
              <br />
              <br />
              <button className={"btn clickable simple"} onClick={clearCartFn}>
                Annuler la{" "}
                {orderDetails?.restaurant?.type === "LIVRAISON"
                  ? "livraison"
                  : "réservation"}
              </button>
            </div>
...
export const AddBankAccount: React.FC<{ closeEvent: Function }> = ({
  closeEvent,
}) => {
  const deviceWidth = useAppSelector(selectDeviceWidth);
  return (
